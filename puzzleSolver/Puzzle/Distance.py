...
def shape_only_edge_compute(e1, e2, debug=False):
    """
    Return the distance between two edges using only shape matching (no color).
    Used for black-only puzzles where all pieces are the same color.
    ...
    :return: distance Float
    """
    # Check if edges have similar length (within 20%)
    length_similar = have_edges_similar_length(
        e1.shape[0], e1.shape[-1], e2.shape[0], e2.shape[-1], 0.20
    )
    if not length_similar:
        if debug:
            print(f"[BLACK_ONLY] shape_only_edge_compute: edges have different lengths, returning inf")
        return float("inf")
    
    # Compute shape difference
    edge_shape_score = diff_match_edges2(np.array(e1.shape), np.array(e2.shape))
    
    # Apply sigmoid to normalize the score
    K = -1.05
    edge_shape_score = (K * edge_shape_score) / (K - edge_shape_score + 1)
    
    # Compute length difference penalty
    shapevalue, distvalue = dist_edge(
        e1.shape[0], e1.shape[-1], e2.shape[0], e2.shape[-1]
    )
    
    # Return shape-based distance (no color component)
    final_score = edge_shape_score * (1.0 + math.sqrt(shapevalue) * 0.3)
    
    if debug:
        print(f"[BLACK_ONLY] shape_only_edge_compute: edge_types=({e1.type.name}, {e2.type.name}), "
              f"shape_score={edge_shape_score:.4f}, shapevalue={shapevalue:.4f}, "
