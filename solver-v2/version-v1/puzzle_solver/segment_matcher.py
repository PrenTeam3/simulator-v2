...
from .utils import normalize_vector, normalize_and_center, rotate_points, find_min_area_rotation, calculate_centroid
...
class SegmentMatch:
    ...
    direction_score: float  # Arrow direction compatibility score
    description: str
...
class SegmentMatcher:
    ...
    @staticmethod
    def _calculate_curvature(points: List[Point]) -> np.ndarray:
        ...
        for i in range(len(points)):
            ...
            if denom > 1e-10:
                curvatures.append(cross / np.sqrt(denom))
            else:
