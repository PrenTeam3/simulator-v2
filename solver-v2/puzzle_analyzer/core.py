...
from puzzle_analyzer.svg_smoother import SVGSmoother
...
class PuzzleAnalyzer:
    ...
    def __init__(self, image_path: str, temp_dir: str = None, strictness: str = 'ultra_strict_minus', debug: bool = False):
        ...
        self.threshold_image = None  # Store threshold image for visualization
    ...
    def detect_pieces(self, min_area: int = 500) -> list:
        ...
        # Store threshold image for visualization
        self.threshold_image = thresh.copy()
    ...
    def save_threshold_image(self, filename: str = "threshold.png"):
        """
        Save the threshold image (black and white contrast).

        Args:
            filename: Filename to save (saved in temp directory)
        """
        if self.threshold_image is None:
            print("Warning: Threshold image not available")
            return None
        output_path = os.path.join(self.temp_dir, filename)
        cv2.imwrite(output_path, self.threshold_image)
...
def analyze_puzzle(image_path: str, temp_dir: str = None, strictness: str = 'ultra_strict_minus', debug: bool = False, target_frame_corners: int = 4):
    ...
    # Save threshold image for preprocessing visualization
    analyzer.save_threshold_image("threshold.png")
